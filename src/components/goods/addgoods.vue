    ...
    <el-form label-position="top" label-width="80px" :model="goodList">
        ...
        <el-tabs tab-position="left" v-model='active' @tab-click='tabChange()'>
            ...
            <el-tab-pane label="商品参数" name='2'>
                <el-form-item v-for='(item1,index) in arrManyParams' :key='index' :label='item1.attr_name'>
                    <el-checkbox-group v-model='item1.attr_vals'>
                        <el-checkbox v-for='(item2,index) in item1.attr_vals' :key='index' :label='item2' border @change='clearParams(index)'>
                            {{index}}{{item2}}
                        </el-checkbox>
                    </el-checkbox-group>
                </el-form-item>
            ...
            <el-tab-pane label="商品属性" name='3'>
                <el-form-item v-for='(item,index) in arrOnlyParams' :key='index' :label='item.attr_name'>
    ...
    methods: {
        ...
        async tabChange() {
            ...
            if (this.active == '2') {
                ...
                // console.log(this.arrOnlyParams)
            }
            if (this.active == '4') {
                console.log(this.arrManyParams)
                console.log(this.arrOnlyParams)
        ...
        async btngoods() {
            ...
            let paramasMany = this.arrManyParams.map((item) => {
                return {
                    "attr_id": item.attr_id,
                    "attr_value": item.attr_vals.join(',')
                }
            })
            let paramasOnly = this.arrOnlyParams.map((item) => {
                return {
                    "attr_id": item.attr_id,
                    "attr_value": item.attr_vals
                }
            })
            this.goodList.attrs = [...paramasMany, ...paramasOnly]
